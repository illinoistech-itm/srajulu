...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: lz4comp.py <file> <dest> <write format>", file=sys.stderr)
        sys.exit(-1)
    ...
    decade=sys.argv[1]
    appName="srajulu convert " + sys.argv[1] + " to " + sys.argv[3]
    s3src='s3a://itmd521/' + sys.argv[1]
    s3dest='s3a://srajulu/' + sys.argv[2]
    ...
    splitDF.write.format(writeformat).mode("overwrite").option("header","true").option("compression","lz4").save(s3dest)
