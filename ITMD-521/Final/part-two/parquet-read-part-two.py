...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        print("Usage: parquet-read-part-two.py <file> ", file=sys.stderr)
        sys.exit(-1)
    ...
    spark = SparkSession.builder.appName("parquet read Part2").config('spark.driver.host','192.168.172.45').config(conf=conf).getOrCreate()
    

    parquet_file_80 = "s3a://srajulu/80.parquet"
    ...
    parquet_df = spark.read.parquet(parquet_file_80,schema=file_scehma)
