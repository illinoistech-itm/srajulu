...
if __name__ == "__main__":
    if len(sys.argv) != 1:
        print("Usage: csv-read-part-two.py <file> ", file=sys.stderr)
        sys.exit(-1)
    ...
    spark = SparkSession.builder.appName("csv read Part2").config('spark.driver.host','192.168.172.45').config(conf=conf).getOrCreate()
    

    csv_file_80 = "s3a://srajulu/80.csv"
    ...
    csv_df = spark.read.csv(csv_file_80,header=True,schema=file_scehma)
